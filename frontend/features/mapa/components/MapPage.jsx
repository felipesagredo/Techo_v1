...
import { COLOR_OPTIONS, getColorOption, getMarkerIcon, refreshMarkers, validateClientCoordinates } from '../hooks/mapHelpers'
...
const MapPage = ({ onBack }) => {
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      ...
      const addressList = await mapService.fetchAddresses(token, API_BASE)
  ...
  const handleUpdateAddress = useCallback(async () => {
    ...
    if (!editingLabel.trim()) {
      showNotification('warning', 'Nombre requerido', 'Escribe un nombre para actualizar la ubicación.')
      return
    }

    try {
      await mapService.updateAddress(
        editingAddress.id,
        {
      ...
      closeEditModal()
      showNotification('success', 'Ubicación actualizada', 'El cambio ya se refleja en el mapa.')
      await loadAddresses()
    } catch (err) {
  ...
  const handleDeleteAddress = useCallback(async () => {
    ...
    try {
      await mapService.deleteAddress(deletingAddressId, token, API_BASE)
  ...
  const handleSubmitLocation = async () => {
    ...
    try {
      ...
      await mapService.createAddress({
